...
def process_dataset(images_dir, dot_map_dir, out_dir):
    ...
    for img_path in img_files:
        ...
        dot_map = np.load(os.path.join(dot_map_dir, img_basename + '.npy'))
        points_tuple = np.where(dot_map==1)
        points = [[y_coord, x_coord] for x_coord, y_coord in zip(points_tuple[0], points_tuple[1])]
...
if __name__=="__main__":
    ...
    # train dataset        
    images_dir = os.path.join(root,'train_data','images') 
    dot_map_dir = os.path.join(root,'train_data','ground_truth_localization')
    out_dir = os.path.join(root,'train_data','gt_map_custom2')
    # scale = 1    

    if(not os.path.isdir(out_dir)):
        os.mkdir(out_dir)

    process_dataset(images_dir, dot_map_dir, out_dir)

    ###############################################################################################
    # test dataset        
    # images_dir = os.path.join(root,'part_A/test_data','images') 
    # mat_dir = os.path.join(root,'part_A/test_data','ground-truth')
    # out_dir = os.path.join(root,'part_A/test_data','gt_map_custom2')
    # scale = 1    
