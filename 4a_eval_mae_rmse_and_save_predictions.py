...
'''
Generate model predictions and calculate mae and rmse
...
The configurations include:
    models_root_dir = directory path containing saved model.
...
Output files:
    out.txt: stats per image and over all images.
    *_e_centers.png: image visualization of predicted dots (centers of connected components).
    *_raw.npy: numpy of raw predictions before applying sigmoid
    *_likelihood.npy: numpy of likelihood predictions after applying sigmoid
    *_hard*.npy: numpy of thresholded predictions or topological map.
...
def run_eval_model(model_param_path, test_loader, out_dir, log_file, thresh_low=0.4, thresh_high=0.5, size_thresh=-1, model_name='topocount_best'):
    ...
    print('model-name', model_name, 'mae', mae, 'rmse', rmse, '2rmse+mae',2*rmse+mae, 'pos_err', pos, 'neg_err', neg)
    log_file.write("model-name {} mae {} rmse {} 2rmse+mae {} pos_err {} neg_err {} \n".format(model_name, mae, rmse, 2*rmse+mae, pos, neg))
    log_file.flush()
    sys.stdout.flush();
...
if __name__=="__main__":
    ...
    out_dir = './eval/sh_parta_custom_topo1_patch50_topocount_test'
    out_filename = 'out.txt'
    ...
    out_dir = './eval/qnrf_custom_topo1_patch100_topocount_test'
    out_filename = 'out.txt'
    ...
    out_dir = './eval/jhu_custom_topo1_patch100_topocount_val'
    out_filename = 'out.txt'
    ...
    out_dir = './eval/jhu_custom_topo1_patch100_topocount_test'
    out_filename = 'out.txt'
    root = './datasets/jhu/jhu_crowd_v2.0'
    ...
    out_dir = './eval/nwpu_custom_topo1_patch100_topocount_val'
    out_filename = 'out.txt'
    ...
    out_dir = './eval/nwpu_custom_topo1_patch100_topocount_test'
    out_filename = 'out.txt'
    root = './datasets/nwpu-crowd'
    test_image_root = os.path.join(root,'images')
    test_dots_root = os.path.join(root,'ground-truth_dots') 
    ...
    model_param_path = os.path.join(models_root_dir, model_filename)
